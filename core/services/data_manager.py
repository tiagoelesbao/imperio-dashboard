...
class ImperioDataManager:
    ...
    def __init__(self, product_id: str = "68ff78f80d0e097d617d472b"):
    ...
    def save_collection_data(self, db: Session, collection_result: Dict) -> bool:
        ...
        try:
            ...
            if "error" in collection_result:
                self._log_collection(db, today, "error", collection_result.get("error"), None)
                return False
            ...
            # 3. Log da coleta
            self._log_collection(db, today, "success", f"ROI: {totals.get('roi', 0):.2f}", totals)
    ...
    def _save_channel_data(self, db: Session, today: date, channels: Dict):
        """Criar novos registros de canal a cada coleta (histórico completo)"""
        ...
        for channel_name, channel_data in channels.items():
            ...
            # SEMPRE criar novo registro - não sobrescrever
            # Isso permite histórico completo de todas as coletas por canal
            ...
            logger.info(f"📝 Novo registro de canal criado para {channel_name}: Vendas=R$ {channel_data.get('sales', 0.0):,.2f}, ROI={channel_data.get('roi', 0.0):.2f}")

    def _log_collection(self, db: Session, today: date, status: str, message: str, totals: Dict = None):
        ...
        log = CollectionLog(
            ...
            total_sales=totals.get("sales", 0.0) if totals else 0.0,
            total_spend=totals.get("spend", 0.0) if totals else 0.0,
            general_roi=totals.get("roi", 0.0) if totals else 0.0
        )
        db.add(log)
